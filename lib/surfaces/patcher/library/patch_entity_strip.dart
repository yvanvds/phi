...
import '../../../engine/state/patch_tree_node.dart';

/// The Patcher surface's collapsible **entity strip** (issue #224, design
/// `docs/design/patcher.md` §3, §8) — a left sidebar mirroring the MIDI library
/// panel. It renders the `patch.` namespace as a tree: groups as folders,
/// patches ordered within them.
///
/// - **Selection** (tap a patch) **opens** it in the editor — its live native
///   patcher becomes the edited one, so the canvas swaps to it (one open patcher
///   at a time in v1).
/// - **Context menu** (the `⋯` button, or a right-click) offers new patch · new
///   group · duplicate · rename · delete; delete routes through the delete-impact
///   dialog when the patch is still referenced.
/// - **Drag** a row onto a group to re-parent it, onto the open strip body to
///   un-group it, or onto a sibling to reorder within a section.
///
/// All behaviour is driven through the [PatchLibraryController]; the strip itself
...
class PatchEntityStrip extends StatefulWidget {
  const PatchEntityStrip({
  ...
  /// Whether the strip starts expanded. Defaults to collapsed (a thin strip).
  final bool initiallyExpanded;

  /// Expanded strip width — matches the reference panel's sibling closely enough
  /// to read as its mirror.
  static const double expandedWidth = 220;

  /// Collapsed strip width — the same affordance as the MIDI library panel.
  static const double collapsedWidth = 28;

  /// Key on the expand / collapse toggle.
  static const Key expandToggleKey = Key('PatchEntityStrip.expandToggle');

  /// Key on the header's `+` add-menu button.
  static const Key addMenuKey = Key('PatchEntityStrip.addMenu');

  /// Key on a node's row, by address — so tests can tap to open or find it.
  static Key rowKey(EntityAddress address) =>
      Key('PatchEntityStrip.row.${address.format()}');

  /// Key on a row's context-menu (`⋯`) button, by address.
  static Key menuKey(EntityAddress address) =>
      Key('PatchEntityStrip.menu.${address.format()}');

  /// Key on a row's drag handle, by address.
  static Key dragHandleKey(EntityAddress address) =>
      Key('PatchEntityStrip.drag.${address.format()}');

  @override
  State<PatchEntityStrip> createState() => _PatchEntityStripState();
}

class _PatchEntityStripState extends State<PatchEntityStrip> {
  late bool _expanded = widget.initiallyExpanded;

  PatchLibraryController get _controller => widget.controller;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: _expanded
          ? PatchEntityStrip.expandedWidth
          : PatchEntityStrip.collapsedWidth,
  ...
  Widget _buildCollapsed() {
    return Column(
      children: [
        _IconButton(
          buttonKey: PatchEntityStrip.expandToggleKey,
          icon: Icons.chevron_right,
          tooltip: 'show patches',
  ...
  List<Widget> _buildRows(PatchTreeNode node, int depth) {
    if (node.isGroup) {
      return [
        _GroupRow(
          node: node,
    ...
    return [
      _PatchRow(
        node: node,
        depth: depth,
        open: _controller.openAddress == address,
        onOpen: () => _controller.open(address),
        onMenu: (position) => _openRowMenu(node, position),
        onDropOnRow: (dragged) {
  ...
  Future<void> _openAddMenu({required EntityAddress? group}) async {
    ...
    switch (choice) {
      case _AddAction.newPatch:
        _controller.newPatch(group: group);
      case _AddAction.newGroup:
        _controller.newGroup(group: group);
    }
  }

  Future<void> _openRowMenu(PatchTreeNode node, Offset globalPosition) async {
    final overlay =
        Overlay.of(context).context.findRenderObject() as RenderBox?;
    if (overlay == null) return;
    ...
    // A patch's new-patch / new-group create a sibling (under its parent); a
    // group's create inside it. Duplicate is patch-only.
    ...
    switch (action) {
      case _RowAction.newPatch:
        _controller.newPatch(group: createGroup);
  ...
  Future<void> _renameNode(PatchTreeNode node) async {
  ...
  Future<void> _deleteNode(PatchTreeNode node) async {
    final impact = _controller.impactOf(node.address);
    final what = node.isGroup ? 'group' : 'patch';
...
/// The expanded strip header: the PATCHES title, an add `+`, and the collapse
/// toggle.
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _IconButton(
            buttonKey: PatchEntityStrip.addMenuKey,
            icon: Icons.add,
            tooltip: 'new…',
            onTap: onAdd,
          ),
...
/// A group folder row — a drop target that re-parents a dragged node into it and
/// a context menu.
...
/// A patch leaf row — tap to open in the editor, drag to reorder / regroup, and a
/// context menu.
      ...
      builder: (context, candidate, rejected) {
        return GestureDetector(
          ...
          child: _RowShell(
            ...
            label: node.name,
            labelColor: open ? PhiColors.fg0 : PhiColors.fg1,
          ),
        );
      },
    );
...
class _RowShell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        key: PatchEntityStrip.rowKey(address),
        height: 26,
        ...
        child: Row(
          children: [
            ...
            _IconButton(
              buttonKey: PatchEntityStrip.menuKey(address),
  ...
  Widget build(BuildContext context) {
    return Draggable<EntityAddress>(
      ...
      child: MouseRegion(
        ...
        child: Icon(
          Icons.drag_indicator,
          key: PatchEntityStrip.dragHandleKey(address),
          size: 14,
          color: PhiColors.fg3,
        ),
      ),
...
/// A compact square icon button styled to the strip's chrome.
...
/// The vertical PATCHES label shown on the collapsed strip.
...
/// The empty-tree hint shown when the `patch.` namespace has no patches yet.
...
/// Modal that renames a patch or group. Owns its [TextEditingController] so the
