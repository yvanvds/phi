...
import 'note_predicate_editor.dart';
...
/// once its declarative [VelocityCurve] model landed (issue #108), and
/// [ConditionalMutingTransform] once its declarative [NoteCondition] model
/// landed (issue #109) — so no transform is callback-driven any more.
bool hasTypedEditor(MidiTransform transform) =>
    ...
    transform is ConditionalMutingTransform;
...
Widget? buildTypedParamEditor(MidiTransformChain chain, int index) {
  ...
  return switch (transform) {
    ...
    ConditionalMutingTransform() => NotePredicateEditor(
      chain: chain,
      index: index,
    ),
    _ => null,
